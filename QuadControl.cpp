...
V3F QuadControl::RollPitchControl(V3F accelCmd, Quaternion<float> attitude, float collThrustCmd)
{
  ...
  if (collThrustCmd > 0.f) {
	  float c_acc= collThrustCmd / mass;
	  float b_x_c = -CONSTRAIN(accelCmd.x / c_acc, -maxTiltAngle, maxTiltAngle);
	  float b_y_c = -CONSTRAIN(accelCmd.y / c_acc, -maxTiltAngle, maxTiltAngle);
  
      float b_x_dot = kpBank * (b_x_c - R(0,2));
      float b_y_dot = kpBank * (b_y_c - R(1,2));
  
      pqrCmd.x = 1 / R (2,2) * (R(1,0) * b_x_dot - R(0,0) * b_y_dot);
      pqrCmd.y = 1 / R (2,2) * (R(1,1) * b_x_dot - R(0,1) * b_y_dot);
  }
  else{
	  pqrCmd.x = 0;
	  pqrCmd.y = 0;
...
float QuadControl::AltitudeControl(float posZCmd, float velZCmd, float posZ, float velZ, Quaternion<float> attitude, float accelZCmd, float dt)
{
  ...
  float acc = (u_1_bar - Gravity) / R(2,2);
...
V3F QuadControl::LateralPositionControl(V3F posCmd, V3F velCmd, V3F pos, V3F vel, V3F accelCmdFF)
{
  ...
  if (velCmd.mag() > maxSpeedXY) {
	  velCmd = velCmd / velCmd.mag() * maxSpeedXY; 
  }
  ...
  if (accelCmd.mag() > maxAccelXY) {
	  accelCmd = accelCmd / accelCmd.mag() * maxAccelXY;
  }
